...
export class StarshipsService {
  ...
  getOne() {
    return this.http.get<ApiResponseData>(`${this.baseUrl}${9}/`).pipe(map(response => ({
     name: response.name,
  ...
  getAll() {
   return this.http.get<ApisData>(this.baseUrl).pipe(map(response => {
   return  this.result = response.results;
